...
# The most efficient script to calculate b-values using the KMS method is KMS_Improved_DC_algorithm.m. One only needs to input the magnitude sequence.
...
4.  An efficient (the time complexity is O(n*logn)) Matlab code for calculating the b-value of a given magnitude sequence based on the improved KMS method. The script is based on the Divide & Conquer (DC) strategy (Lan et al., 2015). --KMS_Improved_DC_algorithm.m
